...
foreach ($tags as $n => $t) {
	foreach ($t as $n2 => $tag) {
		$tagid = $n * 10 + $n2;
        echo "INSERT INTO artpop_Tag VALUES ";
		echo "($tagid, '$tag')";
		echo ";\n";
	}
}
...
for ($date = $startDate; $date <= $endDate; $date += 86400) {
	for ($h = 0; $h <= 23; $h++) {
        ...
        echo "INSERT INTO artpop_Date VALUES ";
...
for ($date = $startDate; $date <= $endDate; $date += 86400) {
	...
	for ($i = 0; $i < $articlesOnThisDay; $i++) {
		...
		// save this article
		echo "INSERT INTO artpop_Article VALUES ($articleID, 'Article #$articleID', '{$categories[$category][$subcategory]}', '{$category}', 'Author #$author', '{$authors[$author]['experience']}', '{$authors[$author]['department']}', $day, $month, $year);\n";
		
		// add some random tags
		foreach ($tags as $n => $t) {
			if (rand() % 5 != 0) {
				$tagid = $n * 10 + rand(0, count($t)-1);
				echo "INSERT INTO artpop_ArticleTags VALUES ($tagid, $articleID);\n";
			}
		}
		...
		for ($d = $date; $d <= $endDate; $d += 86400) {
			for ($h = 0; $h <= 23; $h++) {
                ...
                echo "INSERT INTO artpop_ArticlePopularity VALUES ";
